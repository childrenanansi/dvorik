...
(function ($) {
  ...
  $.fn.shiftcheckbox = function (opts) {
    ...
    if ($containersSelectAll) {
      $checkboxesSelectAll = $containersSelectAll.filter(':checkbox').
    ...
    if (opts.checkboxSelector) {
      ...
      $containersAll = this.filter(function () {
        ...
        $(this).
          data('childCheckbox',
            $(this).find(opts.checkboxSelector).filter(':checkbox')[0]);
    ...
    if ($checkboxesSelectAll) {
      $checkboxesSelectAll.prop('checked', !$checkboxes.not(':checked').length).
        filter(function () {
